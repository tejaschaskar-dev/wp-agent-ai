...
(function(n) {
  ...
  s.prototype.swap16 = function() {
    ...
    for (var m = g - p, F = l - a, U = Math.min(m, F), k = this.slice(p, g), N = i.slice(a, l), T = 0; T < U; ++T)
      if (k[T] !== N[T]) {
        m = k[T], F = N[T];
        break;
      }
  ...
  function ve(u, i, a, l, p) {
    ...
    if (p) {
      var N = -1;
      for (k = a; k < m; k++)
        if (U(u, k) === U(i, N === -1 ? 0 : k - N)) {
          if (N === -1 && (N = k), k - N + 1 === F) return N * g;
        } else
          N !== -1 && (k -= k - N), N = -1;
    } else
      ...
      if (p + F <= a) {
        ...
        switch (F) {
          ...
          case 3:
            ...
            break;
          case 4:
            U = u[p + 1], k = u[p + 2], N = u[p + 3], (U & 192) === 128 && (k & 192) === 128 && (N & 192) === 128 && (T = (g & 15) << 18 | (U & 63) << 12 | (k & 63) << 6 | N & 63, T > 65535 && T < 1114112 && (m = T));
        }
      }
