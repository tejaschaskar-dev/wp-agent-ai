...
`),D.flush()])),o=await fetch(n+"/git-upload-pack",{method:"POST",headers:{Accept:"application/x-git-upload-pack-advertisement","content-type":"application/x-git-upload-pack-request","Content-Length":`${r.length}`,"Git-Protocol":"version=2",...e},body:r});if(!o.ok)throw o.status===401||o.status===403?new Tt(n,o.status):new Error(`Failed to fetch git refs from ${n}: ${o.status} ${o.statusText}`);const c={};for await(const f of ma(o)){const s=f.indexOf(" "),h=f.slice(0,s),d=f.slice(s+1,f.length-1).split(" ")[0];c[d]=h}return c}async function da(n,t){switch(t.type??"infer"){case"commit":return{kind:"commit",refname:t.value,resolvedOid:t.value};case"branch":return{kind:"refname",refname:`refs/heads/${t.value.trim()}`};case"tag":return{kind:"refname",refname:`refs/tags/${t.value.trim()}`};case"refname":return{kind:"refname",refname:t.value.trim()};case"infer":{const r=t.value.trim();if(r===""||r==="HEAD")return{kind:"refname",refname:"HEAD"};if(r.startsWith("refs/"))return{kind:"refname",refname:r};if(la.test(r))return{kind:"commit",refname:r,resolvedOid:r};const o=`refs/heads/${r}`,c=await he(n,o);if(c)return{kind:"refname",refname:o,resolvedOid:c};const f=`refs/tags/${r}`,s=await he(n,f);if(s)return{kind:"refname",refname:f,resolvedOid:s};throw new Error(`Git ref "${t.value}" not found at ${n}`)}default:throw new Error(`Invalid ref type: ${t.type}`)}}async function he(n,t,e){const r=await Lr(n,t,e),o=[t,`${t}^{}`];for(const c of o){const f=c.trim();if(f in r)return r[f]}return null}async function qr(n,t,e){const r=Buffer.from(await ut([D.encode(`want ${t} multi_ack_detailed no-done side-band-64k thin-pack ofs-delta agent=git/2.37.3 filter 
`),D.encode(`filter blob:none
`),D.encode(`shallow ${t}
`),D.encode(`deepen 1
`),D.flush(),D.encode(`done
`),D.encode(`done
`)])),o=await fetch(n+"/git-upload-pack",{method:"POST",headers:{Accept:"application/x-git-upload-pack-advertisement","content-type":"application/x-git-upload-pack-request","Content-Length":`${r.length}`,...e},body:r});if(!o.ok)throw o.status===401||o.status===403?new Tt(n,o.status):new Error(`Failed to fetch git objects from ${n}: ${o.status} ${o.statusText}`);const c=Hr(o.body),f=await Nr(c),s=Buffer.from(await ut(f.packfile)),h=await it.fromPack({pack:s}),y=h.read;return h.read=async function({oid:d,...w}){const b=await y.call(this,{oid:d,...w});return b.oid=d,b},{idx:h,packfile:lt(s),promisor:!0}}async function ya(n,t){const e=await n.read({oid:t});X(e);const r=await n.read({oid:e.object.tree}),o=[r];for(;o.length>0;){const c=o.pop(),f=await n.read({oid:c.oid});if(X(f),c.object=f.object,f.type==="tree")for(const s of f.object)s.type==="tree"&&o.push(s)}return r}async function cr(n){if(!n)return[];const t=[],e=new Set;for(const r of n.idx.hashes??[]){if(e.has(r))continue;const o=n.idx.offsets.get(r);if(o===void 0)continue;const{type:c,object:f}=await n.idx.readSlice({start:o});c==="ofs_delta"||c==="ref_delta"||f&&(e.add(r),t.push({oid:r,type:c,body:lt(f)}))}return t}async function wa(n,t,e){const r=await n.read({oid:t});X(r);const o=await n.read({oid:r.object.tree});X(o);const c={};for(const f of e){let s=o;const h=f.split("/");for(const y of h){if(s.type!=="tree")throw new Error(`Path not found in the repo: ${f}`);let d=!1;for(const w of s.object)if(w.path===y){try{s=await n.read({oid:w.oid}),X(s)}catch{s=w}d=!0;break}if(!d)throw new Error(`Path not found in the repo: ${f}`)}c[f]=s}return c}async function ga(n,t,e){const r=Buffer.from(await ut([...t.map(y=>D.encode(`want ${y} multi_ack_detailed no-done side-band-64k thin-pack ofs-delta agent=git/2.37.3 
`)),D.flush(),D.encode(`done
`)])),o=await fetch(n+"/git-upload-pack",{method:"POST",headers:{Accept:"application/x-git-upload-pack-advertisement","content-type":"application/x-git-upload-pack-request","Content-Length":`${r.length}`,...e},body:r});if(!o.ok)throw o.status===401||o.status===403?new Tt(n,o.status):new Error(`Failed to fetch git objects from ${n}: ${o.status} ${o.statusText}`);const c=Hr(o.body),f=await Nr(c),s=Buffer.from(await ut(f.packfile));return s.byteLength===0?{idx:await it.fromPack({pack:s}),packfile:new Uint8Array,promisor:!1}:{idx:await it.fromPack({pack:s}),packfile:lt(s),promisor:!1}}async function Gr(n,t){const e=await n.read({oid:t});if(X(e),e.type==="blob")return e.object;const r={};for(const{path:o,oid:c,type:f}of e.object)if(f==="blob"){const s=await n.read({oid:c});X(s),r[o]=s.object}else f==="tree"&&(r[o]=await Gr(n,c));return r}function X(n){if(n.object instanceof Buffer)switch(n.type){case"commit":n.object=M.from(n.object).parse();break;case"tree":n.object=de.from(n.object).entries();break;case"blob":n.object=new Uint8Array(n.object),n.format="content";break;case"tag":n.object=st.from(n.object).parse();break;default:throw new kt(n.oid,n.type,"blob|commit|tag|tree")}}async function*ma(n){const t=await n.text();let e=0;for(;e<=t.length;){const r=parseInt(t.substring(e,e+4),16);if(r===0)break;yield t.substring(e+4,e+r),e+=r}}function Hr(n){if(n[Symbol.asyncIterator])return n;const t=n.getReader();return{next(){return t.read()},return(){return t.releaseLock(),{}},[Symbol.asyncIterator](){return this}}}function lt(n){return n instanceof Uint8Array,Uint8Array.from(n)}const ba=pe.deflate,va=/^[0-9a-f]{40}$/i;async function Ea(n){const t={},e=new TextEncoder;return await Promise.all(n.map(async({oid:r,type:o,body:c})=>{if(!r||c.length===0)return;const f=e.encode(`${o} ${c.length}\0`),s=new Uint8Array(f.length+c.length);s.set(f,0),s.set(c,f.length);const h=await ba(s),y=r.slice(0,2),d=r.slice(2);t[`.git/objects/${y}/${d}`]=h})),t}function xa(n,t,e){const r=(n==null?void 0:n.trim())??"";let o=null;switch(t){case"branch":r&&(o=`refs/heads/${r}`);break;case"refname":o=r||null;break;case"tag":r.startsWith("refs/")?o=r:r&&(o=`refs/tags/${r}`);break;case"commit":o=null;break;default:r.startsWith("refs/")?o=r:va.test(r)?o=null:r&&r!=="HEAD"&&(o=`refs/heads/${r}`);break}const c=o?`ref: ${o}
`:`${e}
`,f=o&&o.startsWith("refs/heads/")?o:void 0,s=f==null?void 0:f.slice(11),h=o&&o.startsWith("refs/tags/")?o:void 0,y=h==null?void 0:h.slice(10);return{headContent:c,branchName:s,branchRef:f,tagName:y}}function Aa(n,{branchName:t,partialCloneFilter:e}){const o=["[core]",`	repositoryformatversion = ${e?1:0}`,"	filemode = true","	bare = false","	logallrefupdates = true","	ignorecase = true","	precomposeunicode = true",'[remote "origin"]',`	url = ${n}`,"	fetch = +refs/heads/*:refs/remotes/origin/*","	fetch = +refs/tags/*:refs/tags/*"];return e&&(o.push("	promisor = true"),o.push(`	partialclonefilter = ${e}`),o.push("[extensions]"),o.push("	partialclone = origin")),t&&o.push(`[branch "${t}"]`,"	remote = origin",`	merge = refs/heads/${t}`),o.join(`
`)+`
`}async function Ba({repoUrl:n,commitHash:t,ref:e,refType:r,objects:o,fileOids:c,pathPrefix:f}){const s={},h=xa(e,r,t);s[".git/HEAD"]=h.headContent,s[".git/config"]=Aa(n,{branchName:h.branchName}),s[".git/description"]=`WordPress Playground clone
`,s[".git/shallow"]=`${t}
